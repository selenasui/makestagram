...
struct UserService {
    ...
    static func posts(for user: User, completion: @escaping ([Post]) -> Void) {
        let ref = Database.database().reference().child(Constants.Reference.posts).child(user.uid)
    ...
    static func timeline(completion: @escaping ([Post]) -> Void) {
        ...
        let timelineRef = Database.database().reference().child(Constants.Reference.timeline).child(currentUser.uid)
        timelineRef.observeSingleEvent(of: .value, with: { (snapshot) in
            ...
            for postSnap in snapshot {
                guard let postDict = postSnap.value as? [String : Any],
                    let posterUID = postDict[Constants.Dict.poster_uid] as? String
                    else { continue }
